...
class XhsDbStoreImplement(HttpStore):
    def __init__(self, **kwargs):
        super().__init__(platform="xhs")

    async def store_content(self, content_item: Dict):
        if not content_item or not content_item.get("note_id"):
            return
    ...
    async def store_creator(self, creator_item: Dict):
        if not creator_item or not creator_item.get("user_id"):
            return
        await super().store_creator(creator_item)
...
class XhsExcelStoreImplement:
    """xhs Excel storage implementation - Global singleton"""
